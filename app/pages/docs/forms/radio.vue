          ...
          <div class="d-flex flex-column gap-3">
            ...
            <EDialog v-model="slotDialogOpen" max-width="420">
              <ECard class="pa-4 d-flex flex-column gap-3">
                <div>
                  <div class="text-subtitle-2">{{ pageText.slotDialogTitle }}</div>
                  <p class="text-body-2 mt-2 mb-0">{{ pageText.slotDialogBody }}</p>
                </div>
            ...
            <EForm v-model="validationState.isValid" validate-on-submit @submit="handleValidationSubmit"
              ...
              <EFormColumn cols="12" class="d-flex flex-column gap-2">
                ...
                <div class="d-flex justify-center">
                  <EChip class="text-caption">{{ validationFeedback }}</EChip>
                </div>
              </EFormColumn>
            </EForm>
          </ECard>
...
const content = computed(() => {
  const resolvedContent = getDocsComponentPageContent('/docs/forms/radio', locale.value)

  if (!resolvedContent) {
    throw createError({ statusCode: 404, statusMessage: 'Radio documentation content not found' })
  }
