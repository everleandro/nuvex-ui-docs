          ...
          <div class="d-flex flex-column gap-3">
            ...
            <EDialog v-model="termsDialogOpen" max-width="420">
              <ECard class="pa-4 d-flex flex-column gap-3">
                <div>
                  <div class="text-subtitle-2">{{ pageText.dialogTitle }}</div>
                  <p class="text-body-2 mt-2 mb-0">{{ pageText.dialogBody }}</p>
                </div>
...
const content = computed(() => {
  const resolvedContent = getDocsComponentPageContent('/docs/forms/switch', locale.value)

  if (!resolvedContent) {
    throw createError({ statusCode: 404, statusMessage: 'Switch documentation content not found' })
  }
...
<EDialog v-model="dialogOpen" max-width="420">
  <ECard class="pa-4 d-flex flex-column gap-3">
    <div>
      <div class="text-subtitle-2">${pageText.value.dialogTitle}</div>
